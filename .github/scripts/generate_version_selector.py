...
1. Generates version-selector.js from the JavaScript template.
   The placeholder "**VERSION_OPTIONS**" is replaced with a list
...
# Root URL used by the generated documentation.

#

# "master" is located directly below this path:
...
# Released versions are placed below it:
...
# Directory containing the HTML generated by Doxygen.

DOCS_DIRECTORY = Path("docs/html")

# The JavaScript template is kept separately from this Python script.

#

# The template contains the special string:
...
# which is replaced by build_javascript() with the generated HTML
...
```
Only tags beginning with "v" are considered. For example:

    v1.2.0
    v1.1.0
    v1.0.0

The "v" prefix is retained here and removed later when constructing
...
```
The currently built version receives the HTML "selected" attribute.
...
left unchanged. This prevents the script from being inserted
multiple times if the build script is run repeatedly.

If an HTML document does not contain </body>, the script reference
...
for html_file in DOCS_DIRECTORY.rglob("*.html"):
    ...
    script = (
        '<script src="version-selector.js">'
        '</script>'
    )
